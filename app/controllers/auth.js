...
exports.ownerOrAdmin = () => async (req, res, next) => {
  try {
    ...
    await checkPermissions(data, next).catch(async () => {
      // Si el usuario en el token no es un admin, comprobamos que pertenezca a el
      const params = req.params
      if (params.id !== userId) {
        await utils.handleError(res, utils.buildErrObject(401, 'UNAUTHORIZED'))
      }
      next()
    })
    // req = matchedData(req)
